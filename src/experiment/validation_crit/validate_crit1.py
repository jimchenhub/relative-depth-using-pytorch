...
def _count_correct(output, target, record):
    for point_idx in range(0,target['n_point']):
        ...
        z_A = output[0,0, y_A.data.int().item(), x_A.data.int().item()].item() #zero-indexed
        z_B = output[0,0, y_B.data.int().item(), x_B.data.int().item()].item()

        assert(x_A.item() != x_B.item() or y_A.item() != y_B.item())

        ground_truth = target['ordianl_relation'][point_idx].item()
...
def evaluate(data_loader, model, criterion, max_n_sample):
    ...
    for i in range(0,n_iters):
        ...
        total_validation_loss += (batch_loss * relative_depth_target['n_point']).item()
