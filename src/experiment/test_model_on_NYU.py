...
def inpaint_pad_output_our(output, img_original_width, img_original_height):
    ...
    padded_output[:,crop:img_original_height-crop, crop:img_original_width-crop].copy_(resize_output)
...
if cmd_params.mode == 'test':
    folder_path = "/home/jim/Documents/NYU/data/test_labels/"
    csv_file_name = os.path.join(folder_path, cmd_params.test_set)
elif cmd_params.mode  == 'validate':
    folder_path = "/home/jim/Documents/NYU/data/val_labels/"
    csv_file_name = os.path.join(folder_path, cmd_params.valid_set)
...
t = transforms.Compose([
    transforms.Resize((network_input_height, network_input_width)),
    transforms.ToTensor(),
...
for i in range(0, n_iter):
    ...
    if cmd_params.mode == 'test':
        ...
        gtz_h5_handle = h5py.File(os.path.join(folder_path+os.path.dirname(data_handle[i]['img_filename']),str(i+1)+'_depth.h5'), 'r') #todo
